...
public:
	...
	node * search(int i, node*p); //find the value need to delete
	node * search(node*p); //find the greatest smallest number
};
void avl_tree::add_node(int i) {
	...
	if (action_node != nullptr) {
		...
		action_node = height_adjustment(action_node->parent, pattern); //check for height descripencies and such
	}
...
void avl_tree::remove_node(int i) {
	node*to_remove = search(i, root); // search for value to remove
	if (to_remove == nullptr) { cout << "Value does not exist" << endl; return; }

	//find node it is replacing
	node* replace = to_remove;
	if (to_remove->l_child != nullptr || to_remove->r_child != nullptr) {
		if (to_remove->l_child == nullptr && to_remove->r_child != nullptr) replace = to_remove->r_child; //goes through r_child if l_child does not exist
		else {
			replace = search(to_remove->l_child); //if not special case, goes through l_child
		}
	}

	to_remove->value = replace->value; //change the value

	//special case
	if (replace->l_child != nullptr) {
		replace->value = replace->l_child->value;
		replace = replace->l_child;
	}

	//isolate the replacing node
	if (replace->child == 0) 
		replace->parent->l_child = nullptr;
	else { 
		replace->parent->r_child = nullptr; 
	}

	int pattern = -1;
	//fix the structure
	node* action_node = height_adjustment(replace->parent, pattern);
	if (action_node != nullptr) {
		cout << "action_node's value =  " << action_node->value << "  Imbalance Pattern  = " << pattern << endl;
...
node* avl_tree::search(int i, node* p) { //looking for node
	if (p == nullptr) return nullptr;
	if (i == p->value) return p; //when i is root
	if (i < p->value) {
		search(i, p->l_child);
	}
	else {
...
node * avl_tree::search(node*p) { //to find the greatest smaller number than removed node
	if (p->r_child == nullptr) return p;
	search(p->r_child);
}
